...
(function() {
    ...
    function prepareFile(filePath, folder, timeout, index) {
        ...
        if (!isLocal) {
            ...
            let file = new IMAGE_CONFIG.File(localPath);
            ...
            mime = getMimeType(ext);
            if (!new IMAGE_CONFIG.File(localPath).exists())
    ...
    MediaSender.send = (channelId, path, timeout) => {
        ...
        try {
            if (Array.isArray(path)) {
                ...
                let tasks = [];
                for (let file of path) tasks.push([prepareFile, file, folder, timeout || 30000]);
                ...
                for (let result of results)
                    if (result) {
                        let uri = IMAGE_CONFIG.Uri.fromFile(new IMAGE_CONFIG.File(result.localPath));
                        uris.push(uri);
                ...
                let intent = new IMAGE_CONFIG.Intent(IMAGE_CONFIG.Intent.ACTION_SEND_MULTIPLE);
                intent.setPackage("com.kakao.talk");
                intent.setType("image/*");
                intent.putExtra(IMAGE_CONFIG.Intent.EXTRA_STREAM, uris);
    
                let channelIdLong = new IMAGE_CONFIG.Long(channelId.toString());
                intent.putExtra("key_id", channelIdLong);
                intent.putExtra("key_type", new IMAGE_CONFIG.Integer(1));
                intent.putExtra("key_from_direct_share", true);
    
                intent.addFlags(IMAGE_CONFIG.Intent.FLAG_ACTIVITY_NEW_TASK);
                ...
                for (let filePath of downloaded)
                    setTimeout(() => {
                        try {
                            let tmpFile = new IMAGE_CONFIG.File(filePath);
                            if (tmpFile.exists()) tmpFile.delete();
                        } catch (delErr) {
                            Log.e("send - file deletion", delErr);
                        }
                    }, 60000);
                ...
                let intent = new IMAGE_CONFIG.Intent(IMAGE_CONFIG.Intent.ACTION_SENDTO);
                intent.setPackage("com.kakao.talk");
                intent.setType(result.mime);
    
                let uri = IMAGE_CONFIG.Uri.fromFile(new IMAGE_CONFIG.File(result.localPath));
                intent.putExtra(IMAGE_CONFIG.Intent.EXTRA_STREAM, uri);
                ...
                if (result.downloaded) {
                    setTimeout(() => {
                        try {
                            let tmpFile = new IMAGE_CONFIG.File(result.localPath);
                            if (new IMAGE_CONFIG.File(result.localPath).exists()) tmpFile.delete();
                        } catch (e) {
                            Log.e("send - file deletion", e);
                        }
                    }, 60000);
                }
                return true;
